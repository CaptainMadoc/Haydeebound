...
function realUpdate(dt)
	...
	if data["load"] == "table" then
		localAnimator.addDrawable({image = "/gunsboundhaydee/ui/ammo.png", position = vec2.add(position, {0, -3}), transformation = matt, fullbright = true, color = {255,255 / ratio1,255 / ratio1,255}}, "overlay")
		shift = 0
	...
	for i,v in pairs(data["magazine"]) do
		if v.parameters.fired then
			localAnimator.addDrawable({image = "/gunsboundhaydee/ui/ammo.png", position = vec2.add(position, {((3.75 / data["maxMagazine"]) * i) + shift, -3.25}), transformation = matt, fullbright = true, color = {255,255 / 2,255 / 2,255}}, "overlay")
		else                                                                                                                                       
			localAnimator.addDrawable({image = "/gunsboundhaydee/ui/ammo.png", position = vec2.add(position, {((3.75 / data["maxMagazine"]) * i) + shift, -3.25}), transformation = matt, fullbright = true}, "overlay")
		end
	end
	
	--Fire mode display
	if data["fireSelect"] then
		localAnimator.addDrawable({image = "/gunsboundhaydee/ui/"..data["fireSelect"]..".png", position = vec2.add(activeItemAnimation.ownerAimPosition(), {0, -1}), fullbright = true}, "overlay")	
	end
