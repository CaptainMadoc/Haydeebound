...
function realUpdate(dt)
	...
	if data["load"] == "table" then
		localAnimator.addDrawable({image = "/ui/warframeattica/ammo.png", position = vec2.add(position, {0, -3}), transformation = matt, fullbright = true, color = {255,255 / ratio1,255 / ratio1,255}}, "overlay")
		shift = 0
	end
	
	for i = 1,data["magazine"] do
		localAnimator.addDrawable({image = "/ui/warframeattica/ammo.png", position = vec2.add(position, {((2.5 / data["maxMagazine"]) * i) + shift, -3.25}), transformation = matt, fullbright = true}, "overlay")
	end
	
	--Fire mode display
	if data["fireSelect"] then
		localAnimator.addDrawable({image = "/ui/warframeattica/"..data["fireSelect"]..".png", position = vec2.add(activeItemAnimation.ownerAimPosition(), {0, -1}), fullbright = true}, "overlay")	
	end
